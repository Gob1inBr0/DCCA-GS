...
def main():
    ...
    p.add_argument("--shuffle-split", action="store_true",
                   help="Random 80/20 anchor split (robust to spatial "
                        "autocorrelation); default is contiguous Morton 80/20.")
    ...
    # Drone flight-line spatial context (pure function of anchor coords ->
    # decoder-recomputable, zero side info). center = centroid, extent = max L2.
    center = anchor.mean(dim=0, keepdim=True)
    extent = (anchor - center).norm(dim=1).max().clamp_min(1e-6)
    up = torch.zeros(3, device=device)
    up[2] = 1.0  # Z is assumed up (drone altitude band); configurable later.
    spatial_r = ((anchor - center).norm(dim=1, keepdim=True) / extent)
    spatial_h = (((anchor - center) * up).sum(-1, keepdim=True) / extent)

    parts = {name: [] for name in
             ("ctx", "mean", "scale", "prob", "q", "feat_q",
              "scaling_q", "offsets_q", "mask",
              "mean_s", "scale_s", "mean_o", "scale_o", "qa", "qs", "qo",
              "formula", "sens", "r_norm", "h_norm")}
    ...
    for s in range(math.ceil(N / MAX_batch_size)):
        ...
        parts["r_norm"].append(spatial_r[start:end])
        parts["h_norm"].append(spatial_h[start:end])
    ...
    feats = {
        ...
        "D": torch.cat(
            [g["formula"], g["ctx"], g["mean"], g["scale"], g["prob"],
             g["mean_s"], g["scale_s"], g["mean_o"], g["scale_o"],
             g["qa"], g["qs"], g["qo"], g["r_norm"], g["h_norm"]],
            dim=-1,
        ),
    }
    if args.shuffle_split:
        perm = torch.randperm(N, device=device)
        ntr = int(N * (1.0 - args.val_fraction))
        tr_idx = perm[:ntr]
        va_idx = perm[ntr:]
    else:
        ntr = int(N * (1.0 - args.val_fraction))
        tr_idx = torch.arange(0, ntr, device=device)
        va_idx = torch.arange(ntr, N, device=device)
    ...
    for name, x in feats.items():
        corr, loss = _fit_corr(
            x[tr_idx], g["sens"][tr_idx], x[va_idx], g["sens"][va_idx],
    ...
    for k in [int(v) for v in args.k_list.split(",")]:
        ...
        corr, loss = _fit_corr(
            xc[tr_idx], g["sens"][tr_idx], xc[va_idx], g["sens"][va_idx],
            args.hidden, args.fit_steps, args.weight_decay, device,
        )
    ...
    summary = {
        ...
        "steps": args.steps,
        "input_sets": rows,
        "split": "shuffle" if args.shuffle_split else "morton",
        "best_mean_corr_non_A": round(best_corr, 4),
        "pass": best_corr >= 0.3,
    }
    summary["delta_D_minus_B_feat"] = round(
        rows["D"]["corr"].get("feat", float("nan"))
        - rows["B"]["corr"].get("feat", float("nan")), 4
    )
    out = Path(args.out)
    ...
    print(f"[sens_replace] delta(D-B) feat r = {summary['delta_D_minus_B_feat']:.4f}")
