#!/bin/bash
# Deep Blending lambda-RD runner (PHG recommended config, 110k).
# One cell = train + compress(baseline) + eval + MLP quant(cd8/rest16).
#
# usage:
#   bash runner_db_lambda_rd.sh <gpu> <scene> <data_dir> <lambda>
set -e
...
MAX_STEPS=110000
UPDATE_UNTIL=45000
TAG=db_${SCENE}_i6_110k_h32_l$(printf '%.3f' "$LAMBDA" | tr '.' 'p')
...
for attempt in $(seq 1 40); do
  echo "ATTEMPT $attempt tag=$TAG lambda=$LAMBDA $(date)"
