...
精度的 MLP 权重量化；第三，训练侧的锚点稀疏 SPA。目标是让模型文件更小，同时
解码完全自包含、逐比特可复现。
...
编码把压缩率做得很高，所以它是我们的参照框架；我们不换表示，也不改它的主链路。
...
一个软得分 a，是掩码的平均；每 100 步做一次 ADMM 投影：硬掩码 z 取 a 加乘子 u
...
实验在两个数据集上做：4-28 无人机场景，1200 张训练图、150 个验证视角；以及
Deep Blending 的 playroom 和 drjohnson，覆盖手持、室内、以物体为中心的不同采集
模态。评估统一用解码后的真实码流，1600 宽、test_every 等于 8，体积按解码必需
载荷计。主结果：4-28 上 110k 迭代加 MLP 量化达到 28.82 dB、5.49 MiB，而 HAC++
论文同场景是 28.31 dB、6.95 MiB——质量高 0.51 dB，体积小约 21%；开 SPA 后到
28.30 dB、3.77 MiB。Deep Blending 上高码率点 playroom 30.73 dB、4.22 MiB，
drjohnson 30.04 dB、6.85 MiB；SPA 低码率点分别是 29.90 dB、0.96 MiB 和 28.76 dB、
1.55 MiB，进入了综述方法没有覆盖的亚 1 MiB 区间。
