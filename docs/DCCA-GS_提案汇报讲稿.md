...
实验我们选了两个数据集，覆盖两种互补的采集模态：4-28 是室外无人机航拍场景，
1200 张训练图像、150 个验证视角；Deep Blending 是室内手持、以物体为中心的
playroom 和 drjohnson。评估统一在解码后的真实码流上进行：1600 宽、test_every
取 8，体积按解码必需载荷计。

先看 4-28 的主结果。110k 迭代加 MLP 量化，我们做到 28.82 dB、5.49 MiB；HAC++
论文同场景的参考点是 28.31 dB、6.95 MiB——质量高 0.51 dB，体积小约 21%，质量
和紧凑性同时拿到。开 SPA 之后进一步到 28.30 dB、3.77 MiB，体积再省约 31%。

Deep Blending 上同样清楚：高码率点 playroom 30.73 dB、4.22 MiB，drjohnson
30.04 dB、6.85 MiB；SPA 低码率点分别到 29.90 dB、0.96 MiB 和 28.76 dB、
1.55 MiB，直接进入现有综述方法没有覆盖的亚 1 MiB 区间。
