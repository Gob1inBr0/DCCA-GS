...
新机器完整配置请优先阅读 [环境配置与交接](环境配置与交接.md)。
...
| pytest | 9.1.1 | 测试（5090 上 37 passed） |
...
- `torch_scatter`（**必须**：vendored `hacplus/scene/gaussian_model.py` 仍会 import；
  安装与 torch 版本匹配的 PyG wheel）
